...
_Static_assert(ROWL_ENGINE_CAPABILITY_PLAYER_CHOICES == 64, "Capability ABI changed");
_Static_assert(ROWL_ENGINE_CAPABILITY_LONG_AUDIO_CONTRACT == 2048, "Capability ABI changed");
_Static_assert(ROWL_ENGINE_CAPABILITY_AUDIO_STREAMING == 8192, "Capability ABI changed");
...
void rowl_c_api_header_compile_contract(void) {
    ...
    /* Faz 5 Dilim 1: streaming observability + volume matrix (additive only). */
    (void)RowlEngine_IsStreaming;
    (void)RowlEngine_GetStreamInfoJson;
    (void)RowlEngine_GetBgmVolume;
    (void)RowlEngine_SetAmbienceVolume;
    (void)RowlEngine_GetAmbienceVolume;
    (void)RowlEngine_SetUiVolume;
    (void)RowlEngine_GetUiVolume;
}
