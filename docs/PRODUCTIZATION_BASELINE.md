...
| C ABI diagnostics and ownership | Partial | Structured results, owner-thread guards and length-aware borrowed strings exist; base version/capability (RowlEngine_GetApiVersion c_api.h:133, RowlEngine_GetCapabilities c_api.h:137) and caller-buffer variants (RowlEngine_GetSaveDirectoryUtf8 c_api.h:146, RowlEngine_GetProfileDirectoryUtf8 c_api.h:151, plus GetLocale, GetChoiceLabelAtUtf8, GetStreamInfoJson and about 20 Utf8 caller-buffer variants) exist — native scope: version/capability run handleless, caller-buffer NULL/0 size-query and BUFFER_TOO_SMALL contract holds; C# scope: P/Invoke reads outRequiredSize NUL-included via caller-buffer and carries no borrowed-pointer lifetime; still missing: strict Checked coverage and the diagnosis dome | Phase 1/9 |
